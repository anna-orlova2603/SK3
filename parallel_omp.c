...
int main(int argc, char *argv[]){
    ...
    {
        x_size[0] = procRow[0] + procRow[1] * coord[0]; //procRow[0] * (procRow[1] + 1) + (coord[0] - procRow[0]) * procRow[1]; //
        x_size[1] = procRow[0] * (procRow[1] + 1) + (coord[0] - procRow[0] + 1) * procRow[1];
        y_size[0] = procCol[0] * (procCol[1] + 1) + (coord[1] - procCol[0]) * procCol[1];
        y_size[1] = procCol[0] * (procCol[1] + 1) + (coord[1] - procCol[0] + 1) * procCol[1];
    }
    ...
    do{
        ...
        //r = A*w - B
        #pragma omp parallel for firstprivate(i, j) shared(r2)
        ...
        //A*r
        #pragma omp parallel for firstprivate(i, j) shared(Ar2)
        for(int i = x_size[0]; i < x_size[1]; i++)
        {
            for (int j = y_size[0]; j < y_size[1]; j++)
            {
        ...
        //r_k+1
        #pragma omp parallel for firstprivate(i, j) shared(tau_local_0, tau_local_1)
        for(int i = x_size[0]; i < x_size[1]; i++)
        {
            for (int j = y_size[0]; j < y_size[1]; j++)
            {
                tau_local_0 += iterPar(Ar, r, i, j, h1, h2);
                tau_local_1 += iterPar(Ar, Ar, i, j, h1, h2);
            }
        }

        MPI_Allreduce(&tau_local_0, &tau_global[0], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(&tau_local_1, &tau_global[1], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

        tau = tau_global[0] / tau_global[1];
        
        #pragma omp parallel for private(i, j) //shared(wk, error, w, r2, Ar2)
        for(int i = 0; i <= M; i++)
        {
            for(int j = 0; j <= N; j++)
            {   
                wk[i * (N + 1) + j] = (w[i * (N + 1) + j] - tau * r[i * (N + 1) + j]/1.0);
                if (i == 0 || i == M || j == N)
                {
                    error[i * (N + 1) + j] = 0.0;
                }
        ...
        #pragma omp parallel for firstprivate(i, j) shared(eps_local)
        for(int i = x_size[0]; i < x_size[1]; i++)
        {
            for (int j = y_size[0]; j < y_size[1]; j++)
            {
